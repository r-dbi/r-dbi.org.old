...
The new `Id()` function constructs identifiers. All arguments must be named, yet *DBI* doesn't specify the argument names, because DBMS have a different notion of namespaces. The objects returned by `Id()` are "dumb", they gain meaning only when used in methods such as `dbQuoteIdentifier()` or `dbWriteTable()`.
...
In addition to `dbReadTable()` and `dbWriteTable()`, also `dbExistsTable()` and `dbRemoveTable()` support an `Id()` object as table name. The `dbQuoteIdentifier()` method converts these objects to SQL strings. Some operations (e.g. checking if a table exists) require the inverse, the new `dbUnquoteIdentifier()` generic takes care converting valid SQL identifiers to (a list of) `Id()` objects:
